...
import com.mobile.novabox.bean.Subtitle;
import com.mobile.novabox.bean.VodInfo;
import com.mobile.novabox.cache.CacheManager;
import com.mobile.novabox.event.RefreshEvent;
import com.mobile.novabox.player.ExoPlayer;
import com.mobile.novabox.player.MyVideoView;
import com.mobile.novabox.player.TrackInfo;
import com.mobile.novabox.player.TrackInfoBean;
...
public class PlayFragment extends BaseLazyFragment {
    ...
    private void initView() {
        ...
        mController.setListener(new VodController.VodControlListener() {
            ...
            public void selectEpisode() {
                // 在线视频选集:从 mVodInfo 取当前 playFlag 的剧集列表弹窗
                if (mVodInfo == null || mVodInfo.seriesMap == null || mVodInfo.playFlag == null) return;
                java.util.List<VodInfo.VodSeries> seriesList = mVodInfo.seriesMap.get(mVodInfo.playFlag);
                if (seriesList == null || seriesList.isEmpty()) return;
                java.util.List<String> names = new java.util.ArrayList<>();
                for (VodInfo.VodSeries s : seriesList) names.add(s.name);
                android.app.Activity a = getActivity();
                if (a == null) return;
                new EpisodeSelectDialog(a,
                        (mVodInfo.name == null ? "" : mVodInfo.name) + " 选集",
                        names, mVodInfo.playIndex, index -> {
            ...
            @Override
            public void onPlayerSelected(int newType) {
                try {
            ...
            public void requestExitFullscreen() {
                if (exitFullscreenRequestCallback != null) {
                    exitFullscreenRequestCallback.run();
                }
            }
        });
        mVideoView.setVideoController(mController);
        // 在线视频:显示弹幕设置按钮
        if (mController != null) mController.setDanmuButtonVisible(true);
    }
    ...
    void selectMySubtitle() throws Exception {
        ...
        // 内置字幕切换仅 IJK 播放器支持，IJK 已移除，始终隐藏该入口
    ...
    private void initSubtitleView() {
        ...
        } else if (playSubtitle != null && playSubtitle .length() > 0) {
    ...
    private void searchDanmu(String danmaku) {
        ...
        DanmakuApi.search(mVodInfo.name, series == null ? "" : series.name, new DanmakuApi.SearchCallback() {
            ...
            public void onNotFound() {
                if (!TextUtils.equals(key, progressKey)) return;
                Toast.makeText(mContext, "加载弹幕失败", Toast.LENGTH_SHORT).show();
                checkDanmu("");
            }
        });
    }
...
//        mVodInfo = (VodInfo) bundle.getSerializable("VodInfo");
    ...
    void initPlayerCfg() {
        ...
        try {
            if (!mVodPlayerCfg.has("pl")) {
                mVodPlayerCfg.put("pl", (sourceBean.getPlayerType() == -1) ? (int)Hawk.get(HawkConfig.PLAY_TYPE, 0) : sourceBean.getPlayerType());
            }
    ...
    /**
     * 由 Activity 在真正进入/退出全屏(toggleFullPreview)时调用,用来切换
     * VodController 自身控件条的显隐职责:
     * - isFullscreen=true(真全屏):VodController 自身的进度条/按钮条生效,
     *   默认隐藏,点击屏幕才显示,和 miniControlsOverlay 完全互斥。
     * - isFullscreen=false(小屏预览):VodController 自身控件条一律隐藏,
     *   由 Activity 的 miniControlsOverlay 独占显示控制条。
     */
    public void setControllerFullscreenMode(boolean isFullscreen) {
        if (mController != null) {
            mController.setPreviewMode(!isFullscreen);
        }
    }

    /** Activity 提供的"退出全屏"回调,由 VodController 的退出全屏按钮触发 */
    private Runnable exitFullscreenRequestCallback;

    public void setExitFullscreenRequestCallback(Runnable callback) {
        this.exitFullscreenRequestCallback = callback;
    }
    ...
    private java.util.Set<Integer> triedPlayerTypes = new java.util.HashSet<>();  // 记录已尝试过的播放内核(0=EXO硬解 1=EXO软解 2=MPV硬解 3=MPV软解)
    boolean autoRetry() {
        ...
        if (webPlayUrl != null) {
            if (isNetworkError()) {
                // 网络原因访问不了播放地址(IO/超时/服务器不可达):切换播放内核无意义,直接走降级
                LOG.i("echo-autoRetry network error, skip player switch, try next line");
...
//                        errorWithRetry("解析错误", false);
    ...
    private void configWebViewSys(WebView webView) {
        ...
        ViewGroup.LayoutParams layoutParams = false /* 已强制关闭调试网页面板显示 */

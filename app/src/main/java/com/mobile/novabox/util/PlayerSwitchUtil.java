 ...
 * 播放内核自动切换工具。
 *
 * 6 档 PLAY_TYPE:0=EXO硬解, 1=EXO软解, 2=IJK硬解, 3=IJK软解, 4=MPV硬解, 5=MPV软解。
 * 播放失败时按固定顺序 0→1→2→3→4→5 逐个尝试其余内核,跳过当前与已尝试过的,
 * 四个播放场景(点播/直播/本地/OpenList)共用同一套逻辑。
 */
public class PlayerSwitchUtil {

    public static final int MAX_BUILTIN_TYPE = 5;

    /**
     * 计算下一个待尝试的内核档位。
     *
     * @param currentType 当前正在使用的内核档位(0~5)
    ...
    public static int nextPlayerType(int currentType, Set<Integer> tried) {
        if (currentType < 0 || currentType > MAX_BUILTIN_TYPE) currentType = 2;
    ...
    /**
     * 兼容历史 PLAY_TYPE 编码(老 1=IJK, 老 2=EXO),归一化为 6 档新编码。
    ...
    public static int normalizePlayType(int playType) {
        ...
        if (playType < 0 || playType > MAX_BUILTIN_TYPE) return 2; // 兜底 IJK硬解
        return playType;
    }

    /**
     * 根据内核档位返回对应的 IJK 解码标记。
     * 仅 IJK 软解(3)需要"软解码",其余档位统一"硬解码"(EXO/MPV 路径不消费该字段)。
     */
